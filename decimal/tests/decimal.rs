...
#[test]
fn test_println() {
    if let Some(res) = Decimal::try_from("100000000.001") {
        println!("{:?}", res);
    }
    if let Some(res) = Decimal::try_from("321") {
        println!("{:?}", res);
    }
    if let Some(res) = Decimal::try_from(".001") {
        println!("{:?}", res);
    }
    if let Some(res) = Decimal::try_from("00.000") {
        println!("{:?}", res);
    }
}
...
#[test]
fn test_multi() {
    let left = Decimal::try_from("10.1").expect("err");
    let right = Decimal::try_from("1.001").expect("err");

    println!("left + right: {:?}", left + right); 
    // Decimal { number: 111010, decimal_pow: 10000}
}
